// Copied from ../libcloak/test/support/ricochet_server.dart: a Go ricochet
// server started for a test against localnet's PostgreSQL. Changed here: the
// server's operator surface is switched off.
import 'dart:async';
import 'dart:convert';
import 'dart:io';
import 'dart:math';
...
class RicochetTestServer {
  ...
  Future<void> launch() async {
    ...
    // the operator surface listens on 127.0.0.1:9090 unless told otherwise,
    // which is where localnet's ARC is published and where a second test
    // server in the same pack would try to listen too; a test has no use for it
    final config = File('${dataDir.path}/ricochet.yaml')..writeAsStringSync('ops:\n  enabled: false\n');
