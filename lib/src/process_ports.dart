...
class ProcessPorts implements Ports {
  ...
  Future<SpiffyChain> _started(WalletDir dir, CloakConfig config, {SealedStore? sealed}) async =>
      _chain ??= await SpiffyChain.start(dir, config, progress: progress, sealed: sealed, env: env);
  ...
  Future<TransparentSide> transparent(WalletDir dir, CloakConfig config, SealedStore sealed) async =>
      (await _started(dir, config, sealed: sealed)).transparent();
