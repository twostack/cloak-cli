...
`sudo`. It says so if `~/.local/bin` is not on your `PATH`. `sh install.sh --version 0.1.1`
...
cp -R "/Volumes/cloak 0.1.1/cloak-0.1.1-macos-arm64" ~/.local/share/cloak/0.1.1
ln -sf ~/.local/share/cloak/0.1.1/bin/cloak ~/.local/bin/cloak
```
...
```
spctl --assess --type open --context context:primary-signature -v cloak-0.1.1-macos-arm64.dmg
```
...
gh attestation verify cloak-0.1.1-linux-amd64.tar.gz --repo twostack/cloak-cli
```
...
```
tar -xzf cloak-0.1.1-linux-amd64.tar.gz -C ~/.local/share/cloak
ln -sf ~/.local/share/cloak/cloak-0.1.1-linux-amd64/bin/cloak ~/.local/bin/cloak
```
